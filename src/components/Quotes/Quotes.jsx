  ...
  return (
    <>
      ...
      <div className="p-4 flex gap-2 items-center justify-between ">
        <FaCaretLeft
          className="text-sky-600 text-[40px] rounded-lg hover:bg-sky-600 p-1 hover:text-white transition-all ease-in delay-50"
          onClick={prevQuotePage}
        />
        {numbers.slice(startPage - 1, endPage).map((ele, idx) => (
          <p
            key={idx}
            className={`hover:bg-sky-600 p-1 hover:text-white transition-all ease-in delay-50 cursor-pointer rounded-full ${
              currentPage === ele ? "bg-sky-600 text-white" : ""
            }`}
            onClick={() => getPageQuote(ele)}
          >
            {ele}
          </p>
        ))}
